...
\begin{figure}[H]
    ...
    \fbox{\includegraphics[width=\linewidth, height=0.88\textheight, keepaspectratio]{figures/fig-chaine-temps-reel.png}}
    \vspace{0.2cm}
    \caption{Chaîne de traitement temps réel et contribution des étapes}
    \label{fig:chaine-temps-reel}
\end{figure}
...
Ce mécanisme présente une particularité qu'il faut connaître pour l'employer correctement : la redirection porte sur l'identité du participant attribuée lors de l'établissement de la session, et non sur le numéro d'appel du client. Fournir le numéro plutôt que cette identité provoque un rejet pour discordance. Le transfert est par ailleurs terminal du point de vue de la plateforme : une fois abouti, la session temps réel du client prend fin, l'échange se poursuivant directement avec le conseiller. C'est la raison pour laquelle le dossier de l'échange doit être constitué et transmis avant le transfert, et non pendant.
...
Le modèle principal retenu est GPT-4o mini, dont le profil combine un délai avant premier jeton faible, un appel d'outils fiable et une fenêtre de contexte largement suffisante. Gemini 2.5 Flash constitue le premier repli, les deux modèles ayant été évalués sur le même comportement et une bascule de configuration permettant de le placer en tête de chaîne si son profil devenait plus favorable.
...
\hline
1 & GPT-4o mini & Délai avant premier jeton faible, appel d'outils fiable, large fenêtre de contexte. \\
\hline
2 & Gemini 2.5 Flash & Profil comparable, activable en tête de chaîne par configuration. \\
\hline
...
Deux délais encadrent cette étape et leur ordre est un invariant. Le budget accordé au service de recherche est de cinq secondes, celui du client qui l'appelle de neuf secondes. Le second doit rester strictement supérieur au premier. Sans cette condition, l'appelant abandonnerait avant que le service ait pu répondre, et une absence de réponse deviendrait indiscernable d'une lenteur.

\subsubsection{Réparation contextuelle de la requête}

Dans un échange naturel, les requêtes du client sont souvent elliptiques : « Quel est son prix ? » ou « Je veux l'activer » ne désignent rien sans le contexte de la conversation. Une recherche portant directement sur ces termes est privée du sens qui les rendrait exploitable.

La chaîne de récupération intègre donc une étape de réparation préalable. Avant d'interroger le corpus, un modèle rapide reformule l'énoncé en substituant aux pronoms et aux sous entendus les entités correspondantes issues de la mémoire courte de l'échange. La recherche s'opère ainsi sur une requête complète et autonome, ce qui préserve la pertinence des fragments retournés sans alourdir sensiblement la latence perçue.
...
\begin{figure}[H]
    ...
    \setlength{\fboxsep}{0pt}
    \setlength{\fboxrule}{1pt}
    \fbox{\includegraphics[width=\linewidth, height=0.88\textheight, keepaspectratio]{figures/fig-positionnement-fournisseurs.png}}
    \vspace{0.2cm}
    \caption{Positionnement qualitatif des fournisseurs de la chaîne vocale}
    \label{fig:positionnement-fournisseurs}
\end{figure}
...
\hline
Délai avant premier jeton, régime établi & 1,0 à 1,2 s & 1,0 à 1,2 s & Relevé réel, tour hors cache \\
\hline
...
\hline
\textbf{Total perçu en régime établi} & \textbf{1,6 à 2,1 s} & \textbf{1,8 à 2,5 s} & \textbf{Somme} \\
\hline
\end{tabular}
...
Le total en régime établi est celui d'une conversation ordinaire : c'est le rythme auquel le client s'attend à être entendu une fois la session engagée. Le tout premier contact est plus coûteux, car la première connexion du processus porte encore le coût de la négociation avec le fournisseur. Ce surcoût est toutefois soustrait du chemin perçu dès l'ouverture : la salutation est énoncée à partir d'un texte fixe pendant qu'une connexion est préparée en arrière plan. Lorsque l'échange aborde son régime de croisière, le réseau est déjà établi et le client n'en voit jamais l'effet.
...
\begin{figure}[H]
    ...
    \setlength{\fboxsep}{0pt}
    \setlength{\fboxrule}{1pt}
    \fbox{\includegraphics[width=\linewidth, height=0.88\textheight, keepaspectratio]{figures/fig-budget-latence.png}}
    \vspace{0.2cm}
    \caption{Répartition du budget de latence par étape}
    \label{fig:budget-latence}
\end{figure}
...
\begin{figure}[H]
    ...
    \setlength{\fboxsep}{0pt}
    \setlength{\fboxrule}{1pt}
    \fbox{\includegraphics[width=\linewidth, height=0.88\textheight, keepaspectratio]{figures/fig-concurrence.png}}
    \vspace{0.2cm}
    \caption{Gestion de la concurrence et des ressources}
...
La plateforme est déployée selon une organisation par couches, chacune regroupant les éléments qui partagent le même rôle dans le traitement d'une conversation. Le tableau \ref{tab:deploiement} présente cette lecture architecturale.
...
\hline
\textbf{Couche} & \textbf{Éléments représentés} \\
\hline
Couche cliente & Portail web de l'abonné ; console d'administration et de supervision. \\
\hline
Couche périphérique et d'entrée & Passerelle d'API et d'ingress, support des flux temps réel, passerelle téléphonique. \\
\hline
Cluster d'application : orchestration agentique & Processus porteurs des sessions conversationnelles ; assembleur temps réel. \\
\hline
Cluster d'application : intégration & Passerelles d'accès au support, au métier télécom et aux notifications. \\
\hline
Cluster d'application : services de domaine & Contexte et identité ; connaissance et recherche ; décision ; exécution et audit. \\
\hline
Couche de données et de persistance & Stockage relationnel, index documentaire, stockage d'objets, cache en mémoire, bus d'événements. \\
\hline
Systèmes d'entreprise externes & Opérations télécom, plateforme de gestion de tickets, fournisseurs d'intelligence artificielle. \\
\hline
\end{tabular}
\caption{Organisation architecturale du déploiement de la plateforme}
\label{tab:deploiement}
\end{table}

La figure \ref{fig:deploiement} présente l'organisation de la plateforme par couches. Les accès client et back-office sont séparés du cœur applicatif par une couche périphérique qui porte la téléphonie et les flux temps réel. Le cluster d'application regroupe l'orchestration agentique, les services de domaine et les passerelles vers les systèmes métier ; il s'appuie sur une couche de données et sur les systèmes et fournisseurs externes.

\begin{figure}[htbp]
    \centering
    \setlength{\fboxsep}{0pt}
    \setlength{\fboxrule}{1pt}
    \fbox{\includegraphics[width=\linewidth, height=0.88\textheight, keepaspectratio]{figures/fig-deploiement.png}}
    \vspace{0.2cm}
...
Un service de restitution en tire deux tableaux de bord. Le premier suit la santé de la plateforme : services actifs, débit, part de réponses en erreur, latence au quatre vingt quinzième centile et routes les plus lentes. Le second, plus spécifique, suit le comportement de la couche agentique : répartition des verdicts et son évolution, règles qui refusent et qui escaladent le plus, registre des actions par issue, déclencheurs d'escalade. Il reporte également les bascules de fournisseur et les délais de première réponse audible et de premier jeton par langue.

C'est le second qui présente le plus d'intérêt. Le premier décrirait n'importe quel service web, tandis que le second décrit cette plateforme en particulier. La part de verdicts autorisés y constitue l'indicateur le plus révélateur : il se déplace avant tout le reste lorsqu'un seuil, un catalogue ou un système amont change.

Le second tableau de bord fait encore apparaître une limite dans l'information disponible : il indique combien d'appels d'outils une persona a effectués, mais pas ce qu'elle en a fait. Cette information est captée là où elle existe, au moment où la session tient encore l'agent qui parle, et rattachée à la persona concernée. Elle complète ainsi la lecture du registre des décisions, qui montre le verdict rendu mais non le chemin qui y a mené.
...
\begin{figure}[H]
    ...
    \setlength{\fboxsep}{0pt}
    \setlength{\fboxrule}{1pt}
    \fbox{\includegraphics[width=\linewidth, height=0.88\textheight, keepaspectratio]{figures/fig-chronogramme.png}}
    \vspace{0.2cm}
    \caption{Chronogramme d'un échange conversationnel complet}
    \label{fig:chronogramme}
\end{figure}

\section{Interfaces de l'application}

Deux interfaces web complètent la plateforme. Le portail client donne à l'abonné accès à l'assistant et à sa situation. La console back-office donne à l'utilisateur back-office les moyens d'observer et de gouverner le système. Nous présentons ici les écrans qui correspondent aux capacités décrites dans ce rapport, en écartant les pages secondaires qui n'apportent rien à sa compréhension.
...
La figure \ref{fig:ui-overview} présente la vue d'ensemble opérationnelle, point d'entrée de la supervision. Elle réunit l'état des services, les indicateurs d'activité récents et les alertes en cours, de manière à ce qu'un utilisateur back-office puisse constater en un coup d'oeil si la plateforme fonctionne normalement.
...
La figure \ref{fig:ui-decisions} est la plus significative de cet ensemble. Elle présente, pour une action donnée, la proposition formulée, le verdict rendu, l'identifiant de la règle appliquée et sa justification. Elle rend donc directement observable la chaîne déterministe décrite au chapitre de conception. Elle permet ainsi de répondre non seulement à la question de savoir ce qui a été décidé, mais aussi à celle de savoir en vertu de quoi.
...
Dans la figure \ref{fig:ui-policies}, nous présentons la gestion des règles métier. Le catalogue des règles applicables et leurs seuils y sont administrables, ce qui satisfait l'exigence formulée au chapitre précédent : le comportement du système doit pouvoir évoluer sans intervention sur le code.
...
La figure \ref{fig:ui-agents} présente l'onglet agents de la console d'administration. Il rassemble la matrice croisant les personas aux outils mobilisables et une courbe de tendance d'utilisation. La matrice répond en un coup d'oeil à la question de savoir quel agent dispose de quel outil : une case renseignée indique une capacité réellement attachée, une case vide une capacité absente. Elle fait ainsi apparaître les redondances comme les lacunes. Une matrice est préférée à cinq diagrammes individuels, car elle permet de comparer les agents sur une même dimension et de repérer aussitôt une promesse faite au client qui ne reposerait sur aucun service joignable.

\figurePlaceholder{Console d'administration : onglet agents et matrice des capacités}{ui-agents}{Capture d'écran de l'onglet agents de la console d'administration, présentant la matrice croisant les agents spécialisés aux outils mobilisables et une courbe de tendance d'utilisation.}
...
La figure \ref{fig:ui-docker} présente l'environnement conteneurisé en fonctionnement. Les couches décrites au tableau \ref{tab:deploiement} y apparaissent simultanément avec leur état et leur port, ce qui confronte cette lecture architecturale à son exécution réelle.
...
\hline
Attachement des services & Chaque capacité exposée par une persona correspond à un service réellement joignable ; une promesse faite au client n'est jamais portée par un service inaccessible. \\
\hline
...
\subsubsection{Contrôle d'aptitude des agents}

Une persona peut être cohérente avec elle-même et tenir un outil dont le service amont est indisponible : ses instructions ne mentent pas, mais la capacité qu'elle annonce ne peut être satisfaite. Le client entend alors une promesse puis un échec, qui est plus dommageable qu'une orientation immédiate.

La vérification ne s'arrête donc pas à la cohérence interne des instructions. Elle part des outils exposés par chaque persona et vérifie que chacun correspond à un service réellement joignable. Cette famille rejoint la logique d'injection de défaillance : elle exerce le chemin entre la capacité annoncée et le service qui la porte, plutôt que de se contenter d'une déclaration.
...
Cette section distingue trois catégories de valeurs, que nous séparons nettement afin qu'aucune ne soit confondue avec une autre.

La première est constituée des mesures réelles. La qualité de la recherche documentaire a été obtenue au moyen d'un instrument de sondage développé pour le projet et exécuté sur le corpus réel ; elle est reproduite telle qu'obtenue. S'y ajoute un relevé de latence pris sur un appel réel du portail, ainsi que les principales grandeurs relevées en continu par les tableaux de bord : délai avant la première réponse audible, délai avant le premier jeton par langue, verdicts par règle et bascules de fournisseur.

La deuxième est constituée du budget reconstitué. Le tableau de latence de la section précédente comporte des valeurs de configuration exactes et des temps de traitement estimés. Nous les recalibrons ici sur les relevés réels, en distinguant le premier appel du régime établi.

La troisième est ce qui manque encore. Une campagne sur une période représentative, avec exécutions répétées et analyse statistique, n'a pas pu être menée dans le temps imparti. L'instrument existe et fonctionne ; seule la campagne fait défaut.

\subsection{Mesures réelles}

Le relevé suivant a été pris sur un appel réel du portail. À l'ouverture de la session, l'échange commence par la salutation du Triage. Le premier jeton du modèle de langue arrive 3,48 s après l'ouverture de la session, et le premier fragment de synthèse vocale 0,25 s plus tard : le client entend le premier son environ 4,2 s après l'ouverture.

Ce premier appel ne représente pas le régime de croisière. Sur un tour ultérieur, avec une requête hors cache de 3 909 jetons, le délai avant premier jeton est de 1,12 s. L'écart entre ces deux valeurs ne s'explique ni par un chargement préalable du texte (le premier appel n'était pas mis en cache) ni par la longueur du traitement, puisque la requête de 3 909 jetons revient plus vite que celle de 2 536 jetons. Il s'explique par la première connexion HTTPS du processus, qui paie la résolution DNS, l'établissement TCP et la négociation TLS.

La conséquence est une séquence de mesures : le premier appel est pénalisé par la connexion à froid, les appels suivants bénéficient d'une connexion déjà établie. C'est le motif pour lequel la salutation de la session est produite à partir d'un texte fixe, et un amorçage de connexion est lancé en arrière plan pendant qu'elle est prononcée : le temps de la connexion est retiré du chemin perçu par le client.
...
\begin{figure}[H]
    ...
    \setlength{\fboxsep}{0pt}
    \setlength{\fboxrule}{1pt}
    \fbox{\includegraphics[width=\linewidth, height=0.88\textheight, keepaspectratio]{figures/fig-rag-comparaison.png}}
    \vspace{0.2cm}
    \caption{Séparation entre réponses pertinentes et bruit selon la méthode d'évaluation}
    \label{fig:rag-comparaison}
\end{figure}
...
\subsection{Ce qui manque encore}

Une campagne sur une période représentative reste à conduire. Bien que l'instrumentation relève les grandeurs en continu, une analyse statistique complète a dû être différée en raison des contraintes de calendrier, mais également du nombre limité d'utilisateurs en phase de test. Sans essais de montée en charge à grande échelle suscitant une forte concurrence, nous manquons aujourd'hui des données empiriques requises pour certifier formellement la scalabilité et la tolérance aux pannes du système sous charge maximale.
...
\hline
Services d'observabilité & 2 conteneurs \\
\hline
...
\hline
Écrans de la console d'administration & 21 \\
\hline
Écrans du portail client & 17 \\
\hline
...
La gestion de la concurrence a montré comment les conversations et les opérations sont isolées les unes des autres, et comment les garanties d'exécution unique et d'intégrité de la trace résistent à des accès simultanés. L'architecture physique a établi la correspondance entre la conception et les vingt quatre conteneurs effectivement déployés : six pour l'infrastructure, seize pour les services applicatifs et deux pour l'observabilité. Les interfaces ont donné à voir les surfaces produites pour le client comme pour l'utilisateur back-office.
