...
L'architecture repose sur une séparation en couches concentriques, selon le patron dit hexagonal, également appelé architecture en ports et adaptateurs \cite{cockburn, martin}. Ce choix découle directement de deux besoins non fonctionnels établis au chapitre précédent : l'extensibilité, qui exige qu'un changement de fournisseur externe ne se propage pas dans le système, et la testabilité, qui exige que la logique de décision puisse être vérifiée sans dépendre d'un environnement.

Au centre se trouve le domaine métier, qui porte les entités, les objets valeur et les règles, et qui ne dépend de rien : il ignore par construction l'existence des interfaces, des bases de données et des fournisseurs externes. Il déclare en revanche des interfaces abstraites, les ports, exprimant ce dont il a besoin sans dire comment le lui fournir. Autour viennent les adaptateurs, qui les implémentent.

La direction des dépendances s'en trouve inversée : l'infrastructure se conforme au contrat du domaine, et non l'inverse. Remplacer un fournisseur revient dès lors à écrire un adaptateur, sans toucher aux règles.
...
Le domaine déclare treize ports, chacun correspondant à une capacité dont il a besoin et dont il refuse de connaître la réalisation. Ils se répartissent en trois familles.

\begin{itemize}
  \item \textbf{Conduite de la décision.} Les ports de décision, de politique et d'exécution portent respectivement la proposition d'une action candidate, le verdict motivé qui la tranche et sa réalisation idempotente.
  \item \textbf{Accès au métier de l'opérateur.} Les ports de relation client, de facturation, de solde, de paiement, de provisionnement et de supervision réseau donnent accès aux données et aux opérations détenues par les systèmes de l'opérateur.
  \item \textbf{Services transverses.} Les ports de connaissance, d'audit, de notification et de gestion de tickets couvrent la recherche documentaire, l'enregistrement infalsifiable, l'acheminement des avis et le cycle de vie des tickets.
\end{itemize}
...
Les entités possèdent une identité propre qui persiste au delà de leurs valeurs : deux clients restent distincts même si tous leurs attributs coïncident. Le domaine en compte neuf principales : le client, l'intention, le tour de parole, la conversation, la décision, le verdict de politique, l'action, le ticket et le dossier d'escalade, auxquels s'ajoutent l'enregistrement de consentement et l'entrée d'audit.
...
Quatorze schémas coexistent. Les cinq schémas centraux portent la conversation, la connaissance, la décision et la politique, l'exécution et le routage. Les schémas périphériques reproduisent la structure des systèmes de l'opérateur, à savoir la relation client, la facturation, le solde et l'usage, le provisionnement et l'état des cartes SIM, ainsi que la supervision réseau. Trois schémas transverses complètent l'ensemble : l'identité de portail, l'audit et les référentiels.
...
Son fonctionnement suit quatre temps : interpréter la demande à la lumière du contexte, déterminer ce qu'il convient de faire, soumettre cette intention aux contraintes applicables, puis réaliser l'action validée et en restituer le résultat.
...
\hline
Mémoire d'appels antérieurs & Synthèse des échanges précédents & À l'ouverture, si le client a un historique \\
\hline
...
La mémoire courte est celle de la conversation en cours. Elle conserve les tours de parole, l'état de vérification, les compteurs de clarification et de transfert, l'évolution du sentiment et les identités des opérations engagées. Elle disparaît avec la session.

La mémoire longue traverse les appels. Elle permet qu'un client rappelant au sujet d'un incident déjà signalé n'ait pas à tout réexpliquer. Elle est présentée à l'agent sous forme d'une synthèse et non d'un historique brut, ce qui en borne le coût.
...
\begin{itemize}
  ...
  \item \textbf{Ne jamais présenter un souvenir comme un fait actuel.} Une demande consignée comme ouverte l'était au moment où la synthèse a été constituée. Affirmer au client qu'elle l'est encore constitue une assertion sur le présent, qui exige une relecture de l'état courant avant d'être formulée.
  \item \textbf{Ne rien dire lorsqu'il n'y a rien.} Un premier appel et un client ayant désactivé la mémorisation produisent l'un comme l'autre une synthèse vide, qui n'ajoute ni contenu ni comportement.
\end{itemize}
...
L'autorisation repose sur trois rôles internes ordonnés, chacun disposant des droits du précédent augmentés des siens, auxquels s'ajoute le rôle client. Le tableau \ref{tab:rbac} présente la matrice des droits.
...
Ce chapitre a présenté la conception de la plateforme. La vue statique a établi une architecture en ports et adaptateurs, dans laquelle le domaine métier ne dépend d'aucune technologie et déclare treize interfaces exprimant ses besoins, un découpage en contextes métier délimités, et un modèle de données organisé en schémas assumant deux dissymétries explicites entre sources de vérité et projections. La vue dynamique a décrit le traitement d'un tour de parole, la chaîne conduisant à une action sensible, les deux traitements documentaires et les cycles de vie de la conversation et du ticket.
