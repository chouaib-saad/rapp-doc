...
\begin{figure}[htbp]
    ...
    \fbox{\includegraphics[width=0.72\linewidth, height=0.42\linewidth, keepaspectratio]{figures/logo-amsys-consulting.png}}
    \vspace{0.2cm}
    \caption{Logo de la société Amsys Consulting}
    \label{fig:logo-amsys}
\end{figure}
...
& Personnalisation bornée par l'éditeur visuel. Orchestration et règles métier difficilement externalisables. Contrôle limité sur la couche de récupération documentaire. \\
\hline
Parloa
...
Le projet s'est déroulé de mars à août 2026. Nous l'avons organisé en quatre groupes de travaux — cadrage, réalisation, intégration et livraison — qui se chevauchent partiellement, chacun produisant un ensemble cohérent et démontrable plutôt qu'un simple lot de travaux. La figure \ref{fig:gantt} présente le diagramme de Gantt correspondant, et le tableau \ref{tab:phases} précise chaque tâche, sa période et son résultat.

\begin{figure}[htbp]
    \centering
    \setlength{\fboxsep}{0pt}
    \setlength{\fboxrule}{1pt}
    \fbox{\includegraphics[width=\linewidth, height=0.82\textheight, keepaspectratio]{figures/fig-gantt.png}}
    \vspace{0.2cm}
...
\centering
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|p{2.0cm}|p{2.9cm}|p{3.0cm}|p{6.1cm}|}
\hline
\textbf{Groupe} & \textbf{Tâche} & \textbf{Période} & \textbf{Résultat principal} \\
\hline
Cadrage & Initialisation & Mars S1--S2 & Lancement, périmètre et étude de l'existant. \\
\hline
Cadrage & Spécification & Mars S2--Avril S1 & Cahier des besoins validé. \\
\hline
Cadrage & Architecture & Avril S2--Mai S1 & Architecture cible et principes structurants retenus. \\
\hline
Réalisation & Fondations & Avril S1--S2 & Socle exécutable, contextes métier, modèle de domaine et interfaces entre couches. \\
\hline
Réalisation & Chaîne vocale & Mai S1--S2 & VAD, transcription, synthèse, gestion des tours de parole et des interruptions. \\
\hline
Réalisation & Agents & Mai S2--Juin S2 & Agent d'accueil puis agents métier spécialisés. \\
\hline
Réalisation & Décision & Juin S1--S2 & Moteur de décision, registre d'audit et exécution idempotente des actions sensibles. \\
\hline
Intégration & Connaissance & Juillet S1--S2 & Ingestion, index et recherche documentaire ancrée. \\
\hline
Intégration & Services métier opérateur & Juin S2--Juillet S2 & Passerelles facturation, abonnement et technique. \\
\hline
Intégration & Opérateur & Juillet S1--S2 & Tickets puis escalade et planification de rappels. \\
\hline
Livraison & Fiabilisation & Juillet S1--S2 & Tests, validation et optimisations. \\
\hline
Livraison & Portail et console & Août S1--S2 & Interfaces client et administration, observabilité. \\
\hline
Livraison & Rapport & Avril S1--Août S2 & Rédaction continue et rapport final déposé. \\
\hline
\end{tabular}
...
Les groupes se recouvrent partiellement : plusieurs tâches commencent avant la fin de la précédente et certaines se poursuivent au-delà de leur fenêtre nominale. L'architecture cible, par exemple, est arrêtée après le début des fondations, et la rédaction du rapport accompagne l'ensemble du développement avant de culminer en août. L'optimisation de la chaîne vocale, de même, a fait l'objet d'ajustements jusqu'aux dernières semaines, à mesure que les mesures de latence se précisaient.

\section{Processus de développement}

\subsection{Le choix d'un cycle itératif et incrémental}

Nous avons conduit ce projet selon le modèle évolutif \cite{belgacem}, qui repose sur un développement incrémental et itératif. Ce choix ne relève pas d'une préférence méthodologique mais d'une contrainte propre au sujet.

Un cycle séquentiel suppose que le comportement attendu du logiciel puisse être spécifié complètement avant d'être construit. Cette hypothèse ne tenait pas ici. Une part importante des décisions de conception dépendait de grandeurs qui n'existent qu'une fois le composant en fonctionnement : le temps réellement consommé par une étape de la chaîne vocale, la qualité effective d'une recherche documentaire sur un corpus multilingue, le comportement d'un modèle face à une instruction ambiguë. Ces grandeurs ne se déduisent pas d'une étude préalable, elles se mesurent. Le modèle évolutif répond à cette contrainte : il décompose le projet en incréments fonctionnels développés en ordre de priorité, chacun livrant une portion du produit suffisamment complète pour être utilisée et mesurée. Il garantit ainsi une livraison rapide de produits fonctionnels, une collaboration régulière avec le client et une flexibilité face à l'évolution des besoins.

\subsection{La forme concrète des itérations}

Chaque itération a pris la forme d'un incrément numéroté, associé à une branche de développement dédiée et à un document décrivant son contenu, son intention et son résultat. Un incrément traverse toujours les mêmes étapes : spécification, conception, développement et tests, intégration, puis validation.
...
Chaque incrément livré apporte une valeur ajoutée à la version globale du logiciel en ajoutant une fonctionnalité réelle et souhaitée par le client. La figure \ref{fig:cycle-iteratif} illustre le modèle évolutif appliqué au projet : le produit devient utilisable dès le premier incrément, puis sa couverture s'étend au fil des suivants, matérialisée par les jalons de 30 %, 55 %, 80 % et 100 %. Les retours du client, recueillis à partir du premier incrément, orientent l'ajout des fonctionnalités suivantes.

\begin{figure}[htbp]
    \centering
    \setlength{\fboxsep}{0pt}
    \setlength{\fboxrule}{1pt}
    \fbox{\includegraphics[width=\linewidth, height=0.82\textheight, keepaspectratio]{figures/fig-cycle-iteratif.png}}
    \vspace{0.2cm}
    \caption{Cycle de la méthodologie évolutive appliqué au projet}
    \label{fig:cycle-iteratif}
\end{figure}

\subsection{Vérification continue}

La mesure occupant une place centrale dans ce cycle, elle devait être disponible en permanence et non à l'issue du développement. Nous avons donc constitué progressivement un ensemble de vérifications automatisées. Elles couvrent les comportements que le système doit garantir quelles que soient ses évolutions ultérieures : le caractère déterministe des décisions, l'exécution unique des actions sensibles, le refus de répondre en l'absence de source documentaire, le comportement en cas de défaillance d'un fournisseur externe, et le respect de la langue de l'échange. Le chapitre de réalisation revient en détail sur cet ensemble et sur ce que chaque famille de vérifications protège.
